...
vnets := plan_functions.get_resources_by_type("aws_security_group", resource_changes)
...
sg_rules := plan_functions.get_resources_by_type("aws_security_group_rule", resource_changes)

checkPortAllowed(resource_checks, port) = port_violations{
    port_violations := [ resource | 
      resource := resource_checks[_]
      resource.change.after.to_port != port[0]
      resource.change.after.to_port != port[1]
    ]
}
...
deny[msg] {
    ...
    resources != []
    msg := sprintf("To port must be one of the following in prod environments: %v",[prod_allowed_ports])
}
